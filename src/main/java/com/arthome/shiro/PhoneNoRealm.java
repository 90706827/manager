 ...
 **/
@Component
public class PhoneNoRealm extends AuthorizingRealm implements Logger {
    /**
     * 账户禁用
     */
    ...
    public boolean supports(AuthenticationToken token) {
        ...
        return token instanceof PhoneNoToken;
    }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
         ...
         */
        PhoneNoToken phoneNoToken = (PhoneNoToken) token;
        Session session = SecurityUtils.getSubject().getSession();
        String captcha = session.getAttribute("_code").toString();
        User user = userService.selectUserByPhoneNo(phoneNoToken.getPrincipal().toString());
        if (captcha == null || !captcha.equals(phoneNoToken.getCaptchaCode())) {
            session.setAttribute("errorMsg", "验证码不正确！");
            throw new AuthenticationException("验证码不正确！");
        }
        if (user == null){
            session.setAttribute("errorMsg", "手机号未注册！");
            throw new AuthenticationException("手机号未注册！");
        }
        String password = passWordService.encryptPassword(String.valueOf(phoneNoToken.getPassword()), user.getPassSalt());
        ...
        return new SimpleAuthenticationInfo(user, phoneNoToken.getPassword(), getName());
    }
